...
//   children: []
// }
export function flattenSchema(root, name = '#', parent, result = {}) {
  ...
  const children = [];
  const isContainer =
    _root.schema?.ui?.type === 'container' && !!_root.children;

  if (isContainer) {
    _root.children.forEach(value => {
      flattenSchema(value, value.$id, name, result);
  ...
  if (item) {
    ...
    if (item.children.length > 0) {
      ...
      children.forEach(child => {
        ...
        try {
          if (final) {
            childId = flatten[child].$id;
          }
        } catch (error) {
          console.log('catch', error);
        }
        const key = getKeyFromUniqueId(childId);
        if (item.schema.ui.type === 'container') {
          item.children.push(idToSchema(flatten, child, final));
        }
      });
    }
  }
  ...
  try {
    ...
    if (childrenIds && childrenIds.length > 0) {
      if (result === undefined) {
        ...
        if (['container'].indexOf(flatten[id].schema.type) > -1) {
          result = {};
        }
      }
